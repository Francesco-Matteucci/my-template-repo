...
<div class="container">
    ...
    <form action="{{ route('players.update', $player->id) }}" method="POST">
        ...
        <!-- Inizio della riga per le due colonne -->
        <div class="row">
            <!-- Prima colonna -->
            <div class="col-md-6">
            ...
            <div class="col-md-6">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="total_rare_dropped" id="total_rare_dropped" class="form-control" value="{{ $player->total_rare_dropped }}" min="0" readonly>
                </div>

                <div class="mb-3">
                    <label for="total_rare_system" class="form-label text-light">Totale Drop Rari Assegnati da Sistema</label>
                    <input type="number" name="total_rare_system" id="total_rare_system" class="form-control" value="{{ $player->total_rare_system }}" min="0" readonly>
                </div>

                <div class="mb-3">
                    <label for="total_rare_distributed" class="form-label text-light">Totale Drop Rari Distribuiti</label>
                    <input type="number" name="total_rare_distributed" id="total_rare_distributed" class="form-control" value="{{ $player->total_rare_distributed }}" min="0" readonly>
                </div>

                <!-- Campo visualizzato solo per priorità -->
                <div class="mb-3">
                    <label class="form-label text-light">Priorità</label>
                    <p class="text-light">{{ $player->priority }}</p>
                </div>
            </div>
        </div>
        <!-- Fine della riga -->
